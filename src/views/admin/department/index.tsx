import { FC, useEffect, useState } from "react"
import { Plus, Layers, Pencil, Trash2, ArrowLeft } from "lucide-react"
import { useParams, useNavigate } from "react-router-dom"
import { useQueryClient } from "@tanstack/react-query"
import toast from "react-hot-toast"

import useDepartments, { Department } from "../../../hooks/department/useDepartments"
import useDepartmentDelete from "../../../hooks/department/useDepartmentDelete"

import ActionDropdown from "../../../components/ActionDropdown"
import CreateDepartmentModal from "./create"
import EditDepartmentModal from "./edit"

const DepartmentPage: FC = () => {
  const { branchId, divisionId } = useParams()
  const navigate = useNavigate()
  const queryClient = useQueryClient()

  const { data: departments, isLoading, isError, error } =
    useDepartments(branchId!, divisionId!)

  const deleteDepartment = useDepartmentDelete(branchId!, divisionId!)

  const [open, setOpen] = useState(false)
  const [isEditOpen, setIsEditOpen] = useState(false)
  const [selectedDepartment, setSelectedDepartment] =
    useState<Department | null>(null)

  useEffect(() => {
    document.title = "Company - Department Management"
  }, [])

  // ===== DELETE =====
  const handleDelete = (id: number) => {
    if (!confirm("Are you sure you want to delete this department?")) return

    const toastId = toast.loading("Deleting department...")

    deleteDepartment.mutate(String(id), {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["departments", branchId, divisionId],
        })
        toast.success("Department deleted successfully", { id: toastId })
      },
      onError: () => {
        toast.error("Failed to delete department", { id: toastId })
      },
    })
  }

  return (
    <div className="space-y-6">

      {/* HEADER */}
      <div className="flex items-center justify-between">
        <div>
          <button
            onClick={() =>
              navigate(`/admin/branches/${branchId}`)
            }
            className="flex items-center gap-2 text-yellow-600 hover:text-yellow-400 mb-2"
          >
            <ArrowLeft size={14} /> Back to Division
          </button>

          <h1 className="text-2xl sm:text-3xl font-bold text-white">
            Department Management
          </h1>
          <p className="text-gray-400 text-sm mt-1">
            Manage departments inside selected division
          </p>
        </div>

        <button
          onClick={() => setOpen(true)}
          className="flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700 px-4 py-2 rounded-lg transition"
        >
          <Plus size={16} />
          Add Department
        </button>
      </div>

      {/* STATS */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-gray-900 p-4 rounded-xl border border-gray-800 flex items-center justify-between">
          <div>
            <p className="text-gray-400 text-xs">Total Departments</p>
            <h2 className="text-2xl font-bold text-white mt-1">
              {departments?.length || 0}
            </h2>
          </div>

          <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center">
            <Layers className="text-purple-400" size={20} />
          </div>
        </div>
      </div>

      {/* TABLE */}
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 overflow-x-auto">
        {isLoading && <p className="text-gray-400">Loading departments...</p>}

        {isError && (
          <p className="text-red-500">
            {error?.message || "Something went wrong"}
          </p>
        )}

        {!isLoading && !isError && (
          <table className="min-w-full text-sm text-left text-gray-400">
            <thead className="text-xs uppercase bg-gray-800">
              <tr>
                <th className="px-6 py-2">Name</th>
                <th className="px-6 py-2 text-right">Actions</th>
              </tr>
            </thead>

            <tbody>
              {departments?.map((department) => (
                <tr
                  key={department.id}
                  className="border-b border-gray-800 hover:bg-gray-800/40 transition"
                >

                  <td className="px-6 py-1 text-yellow-400 font-medium">
                    {department.name}
                  </td>

                  <td
                    className="px-6 py-1 text-right"
                  >
                    <ActionDropdown
                      items={[
                        {
                          label: "Update",
                          icon: Pencil,
                          onClick: () => {
                            setSelectedDepartment(department)
                            setIsEditOpen(true)
                          },
                        },
                        {
                          label: "Delete",
                          icon: Trash2,
                          danger: true,
                          onClick: () => handleDelete(department.id),
                        },
                      ]}
                    />
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}

        {!isLoading && departments?.length === 0 && (
          <div className="text-center py-10 text-gray-500">
            No departments found.
          </div>
        )}
      </div>

      {/* CREATE MODAL */}
      <CreateDepartmentModal
        branchId={branchId!}
        divisionId={divisionId!}
        isOpen={open}
        onClose={() => setOpen(false)}
        onSuccess={() =>
          queryClient.invalidateQueries({
            queryKey: ["departments", branchId, divisionId],
          })
        }
      />

      {/* EDIT MODAL */}
      <EditDepartmentModal
        branchId={branchId!}
        divisionId={divisionId!}
        department={selectedDepartment}
        isOpen={isEditOpen}
        onClose={() => setIsEditOpen(false)}
        onSuccess={() =>
          queryClient.invalidateQueries({
            queryKey: ["departments", branchId, divisionId],
          })
        }
      />
    </div>
  )
}

export default DepartmentPage